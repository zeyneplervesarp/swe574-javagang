...
public interface ServiceRepository extends JpaRepository<Service, Long> {
    ...
    List<Service> findByDescriptionLikeIgnoreCase(@Param("match") String stringToMatch);

    @Query("select s from Service s where lower(s.header) like lower(concat('%', :match, '%'))")
    List<Service> findByHeaderLikeIgnoreCase(@Param("match") String stringToMatch);

    @Query("select s from Service s where lower(s.location) like lower(concat('%', :match, '%'))")
    List<Service> findByLocationLikeIgnoreCase(@Param("match") String stringToMatch);
}