...
public class PaginatedResponse<T> {
    ...
    public <R> PaginatedResponse(List<T> items, String endpointPrefix, Pagination<R> currentPagination, Function<T, R> lastValueFn) {
        this.items = items;
        if (!items.isEmpty()) {
            final var lastValue = lastValueFn.apply(items.get(items.size() - 1));
            final var nextPage = currentPagination.nextPage(lastValue);
            this.nextPage = nextPage.makeUrlString(endpointPrefix);
        } else {
            this.nextPage = null;
        }
    }
